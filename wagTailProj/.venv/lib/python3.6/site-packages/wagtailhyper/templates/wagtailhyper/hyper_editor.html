{% extends 'wagtailhyper/editor_base.html' %}
{% load static %}
...
<script src="{% static 'wagtailhyper/js/build.js' %}"></script>
...
<script>
    ...
    var WAGTAIL_IMAGE_SEARCH_URL = '{% url "wagtailhyper_api_images" %}'
    ...
    function asyncPreview(value, callback) {
        fetch('{% url "wagtailhyper_partial_render" %}', {
            credentials: 'include',
            method: 'POST',
            headers: {
    ...
    hyperEditor = new HyperEditor.default('#hyperEditor', {
        imageSearchApi: WAGTAIL_IMAGE_SEARCH_URL,
